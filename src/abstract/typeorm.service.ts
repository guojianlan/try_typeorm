...
export interface AbstractTypeOrmServiceOptions {
  deleteMode: 'default' | 'log' | 'time';
}
export abstract class AbstractTypeOrmService<T> {
  protected _model: Repository<T>;
  protected _entity: EntityTarget<T>;
  protected options: AbstractTypeOrmServiceOptions;
  constructor(
    model: Repository<T>,
    _entity: EntityTarget<T>,
    options: AbstractTypeOrmServiceOptions = {
      deleteMode: 'default',
    },
  ) {
    this._model = model;
  ...
  public async create(body): Promise<T[] | InsertResult> {
    ...
    // const result = await builder.insert().values([body]).execute();
    // return result;
  ...
  public async delete(id: number): Promise<boolean> {
    try {
      console.log(this.options.deleteMode);
      if (this.options.deleteMode == 'default') {
        await this.queryBuilder().delete().where({ id: id }).execute();
      }
      if (this.options.deleteMode == 'time') {
      ...
      if (this.options.deleteMode == 'log') {
        //   拷贝一份数据出去
        this.logDelete();
      }
      // 执行后续操作
  ...
  public async logDelete() {
    ...
    const result = await manager.query(
      `CREATE TABLE if not exists targetTable44 LIKE ${this._model.metadata.tableName};`,
    );
    console.log(result);
